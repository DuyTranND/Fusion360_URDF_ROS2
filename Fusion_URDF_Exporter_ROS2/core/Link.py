...
class Link:
    ...
    def make_link_xml(self):
        ...
        inertial_origin_xyz = ' '.join([str(_) for _ in self.center_of_mass])
        origin_i.attrib = {'xyz':inertial_origin_xyz, 'rpy':'0 0 0'}
        ...
        mesh_filename = 'file://' + '$(find %s)' % self.pkg_name + self.remain_repo_addr + self.name + '.stl'
        mesh_v.attrib = {'filename':mesh_filename,'scale':'0.001 0.001 0.001'}
        ...
        collision_mesh_filename = 'file://' + '$(find %s)' % self.pkg_name + self.remain_repo_addr + self.name + '.stl'
        mesh_c.attrib = {'filename':collision_mesh_filename,'scale':'0.001 0.001 0.001'}

        # print("\n".join(utils.prettify(link).split("\n")[1:]))
        self.link_xml = "\n".join(utils.prettify(link).split("\n")[1:])


def make_inertial_dict(root, msg, save_dir=None):
    """      
    Parameters
    ----------
    ...
    for occs in allOccs:
        ...
        center_of_mass_raw = prop.centerOfMass.asArray()
        center_of_mass = [_/100.0 for _ in center_of_mass_raw] ## cm to m
