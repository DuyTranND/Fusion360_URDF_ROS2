...
def _init_link_log(save_dir=None):
    """Initialize log file for Link module"""
    global _link_log_file
    if _link_log_file is not None:
        return
    if save_dir is None:
        save_dir = os.path.expanduser("~")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = os.path.join(save_dir, f"links_debug_{timestamp}.log")
    _link_log_file = open(log_path, 'w', encoding='utf-8')
    _log(f"=== Link Debug Log Started at {datetime.now().isoformat()} ===")
...
def _log(msg):
    ...
    print(msg)
    if _link_log_file:
        _link_log_file.write(msg + "\n")
        _link_log_file.flush()

def _close_link_log():
    """Close the log file"""
    global _link_log_file
    if _link_log_file:
        _log("=== Link Debug Log Ended ===")
        _link_log_file.close()
        _link_log_file = None
...
class Link:
    ...
    def __init__(self, name, xyz, center_of_mass, repo, mass, inertia_tensor):
        ...
        # Log all Link constructor variables
        _log(f"\n--- Link.__init__: {self.name} ---")
        _log(f"  name: {self.name}")
        _log(f"  xyz (input): {xyz}")
        _log(f"  xyz (reversed for visual): {self.xyz}")
        _log(f"  center_of_mass: {self.center_of_mass}")
        _log(f"  repo: {self.repo}")
        _log(f"  pkg_name: {self.pkg_name}")
        _log(f"  remain_repo_addr: {self.remain_repo_addr}")
        _log(f"  mass: {self.mass}")
        _log(f"  inertia_tensor: {self.inertia_tensor}")
